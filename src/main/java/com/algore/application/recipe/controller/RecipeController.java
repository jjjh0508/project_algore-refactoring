...
//            List<RecipeProcedureDTO> recipeProcedureDTOS = recipeWriteDTO.getRecipeProcedureDTOList(); //단계
            List<RecipePhotoWriteDTO> recipePhotoWriteDTOList = new ArrayList<>(); //완성사진
            List<RegistIngredientDTO> recipeIngredientDTOS = new ArrayList<>();
            int recipeNum = recipeWriteDTO.getRecipeNum();

            String path = System.getProperty("user.dir") + "\\src\\main\\resources\\static\\upload\\basic\\";
            //경로 폴더지정
            ...
            if (mainFile != null) {
                ...
                mainFile.transferTo(new File(path + "\\" + mainPhotoName));

                recipeWriteDTO.setMainPhoto(mainPhotoName);
                System.out.println(mainPhotoName);
            ...
            for (int i = 0; i < rpContent.size(); i++) {
                System.out.println("요리 순서 for문");

                //사진이 없으면
                if (rpFile.get(i).getOriginalFilename().equals("")) {
                    System.out.println(rpFile.get(i).getOriginalFilename().equals(""));
                    ...
                    rpFile.get(i).transferTo(new File(path + "\\" + newOrdeFileName));
                    selectProcedures.add(new SelectProcedure(recipeNum, rpContent.get(i), newOrdeFileName, "/upload/basic/")); //요리 순서 모우기
                }

            }

//                // 완성 사진
            for (int i = 0; i < rPcFile.size(); i++) {
                String completeName = rPcFile.get(i).getOriginalFilename();

                if (!completeName.equals("")) {
                    System.out.println(completeName + "2");
            ...
            for (RegistIngredientDTO recipeIngredientDTO : recipeIngredientDTOS) {
            ...
            recipeWriteDTO.setSelectProcedure(selectProcedures);
